...
vec3 phongPuntual(vec3 norm, Luz luz, vec3 posicion, Material mat, float sombreado){
	...
	float b = 0.05f;
	float c = 1.0f;
	float decaimiento = (luz.intensidad.x+luz.intensidad.y+luz.intensidad.z)/3.0f * (1.0f/(b * float(length(luz.posicion.xyz - posicion)) + c));
...
vec2 ParallaxTexCoord(vec2 textureCoord){
	...
	h= texture2D(displacementMap, vec2(v)).x - 0.5f;
...
{	
	...
	vec3 normal = normalize(vec3(texture2D( normalMap, FixedTexCoord )-vec4(0.5f, 0.5f, 0.5f, 0.0f)) * toObjectLocal) * normalFactor;	
	...
	gl_FragColor = vec4(pow(finalColor, vec3(1.0f/gamma)) , 1.0f );
}